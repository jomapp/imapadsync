    ...
    return new Promise<ParsedImapRFC822>((resolve, reject) => {
      ...
      this.parser.on(
        ...
        (
          ...
          parsedImapRFC822.parsedEnvelope =
            ImapMailEnvelope.fromMailParserHeadersMap(headersMap);
        }
      );

      this.parser.on('data', async (data: any) => {
        if (data.type === 'text') {
          parsedImapRFC822.parsedBody = new ImapMailBody(
            data.html,
            data.text,
            data.textAsHtml
          );
        ...
        if (data.type === 'attachment') {
          ...
          if (content.length > 0) {
            ...
            let imapMailAttachment = new ImapMailAttachment(
              data.size,
              data.headers,
              data.contentType,
              content,
