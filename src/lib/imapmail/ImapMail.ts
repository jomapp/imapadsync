import {
  ImapMailRFC822Parser,
  MailParserAddressObject,
} from './ImapMailRFC822Parser.js';
...
export class ImapMailAddress {
  ...
  static fromMailParserAddressObject(
    mailParserAddressObject: any
  ): ImapMailAddress {
...
export class ImapMailEnvelope {
  ...
  static fromMailParserHeadersMap(
    mailParserHeadersMap: Map<
      string,
      string | string[] | MailParserAddressObject | Date | object
    >
  ) {
    ...
    if (mailParserHeadersMap.has('subject')) {
      imapMailEnvelope.setSubject(
        mailParserHeadersMap.get('subject') as string
      );
    }

    if (mailParserHeadersMap.has('message-id')) {
      imapMailEnvelope.setMessageId(
        mailParserHeadersMap.get('message-id') as string
      );
    }

    if (mailParserHeadersMap.has('in-reply-to')) {
      imapMailEnvelope.setInReplyTo(
        mailParserHeadersMap.get('in-reply-to') as string
      );
    }
    ...
    if (mailParserHeadersMap.has('references')) {
      ...
      let references =
        typeof headerReferences === 'string'
          ? [headerReferences]
          : (headerReferences as string[]);
      imapMailEnvelope.setReferences(references);
    }

    if (mailParserHeadersMap.has('from')) {
      imapMailEnvelope.setFrom(
        (mailParserHeadersMap.get('from') as MailParserAddressObject).value.map(
          (from) => ImapMailAddress.fromMailParserAddressObject(from)
        )
      );
    }
    ...
    if (mailParserHeadersMap.has('sender')) {
      imapMailEnvelope.setSender(
        (
          mailParserHeadersMap.get('sender') as MailParserAddressObject
        ).value.map((sender) =>
          ImapMailAddress.fromMailParserAddressObject(sender)
        )
      );
    }
    ...
    if (mailParserHeadersMap.has('to')) {
      imapMailEnvelope.setTo(
        (mailParserHeadersMap.get('to') as MailParserAddressObject).value.map(
          (to) => ImapMailAddress.fromMailParserAddressObject(to)
        )
      );
    }
    ...
    if (mailParserHeadersMap.has('cc')) {
      imapMailEnvelope.setCc(
        (mailParserHeadersMap.get('cc') as MailParserAddressObject).value.map(
          (cc) => ImapMailAddress.fromMailParserAddressObject(cc)
        )
      );
    }
    ...
    if (mailParserHeadersMap.has('bcc')) {
      imapMailEnvelope.setBcc(
        (mailParserHeadersMap.get('bcc') as MailParserAddressObject).value.map(
          (bcc) => ImapMailAddress.fromMailParserAddressObject(bcc)
        )
      );
    }
    ...
    if (mailParserHeadersMap.has('reply-to')) {
      imapMailEnvelope.setReplyTo(
        (
          mailParserHeadersMap.get('reply-to') as MailParserAddressObject
        ).value.map((replyTo) =>
          ImapMailAddress.fromMailParserAddressObject(replyTo)
        )
      );
    }
...
export class ImapMail {
  ...
  static async fromImapFlowFetchMessageObject(
