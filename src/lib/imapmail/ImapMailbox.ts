...
export class ImapMailboxStatus {
  ...
  static fromImapFlowMailboxObject(
    mailboxObject: MailboxObject
  ): ImapMailboxStatus {
    return new ImapMailboxStatus(
      mailboxObject.path,
      mailboxObject.uidNext,
      mailboxObject.uidValidity
    )
...
export class ImapMailbox {
  ...
  static fromImapFlowListTreeResponse(
    listTreeResponse: ListTreeResponse,
    parentFolder: ImapMailbox | null
  ): ImapMailbox {
    ...
    if (listTreeResponse.folders) {
      imapMailbox.setSubFolders(
        listTreeResponse.folders.map((value: ListTreeResponse) =>
          ImapMailbox.fromImapFlowListTreeResponse(value, imapMailbox)
        )
      );
    }
