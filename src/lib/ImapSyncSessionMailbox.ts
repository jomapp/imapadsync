...
export class ImapSyncSessionMailbox {
  ...
  importance: SyncSessionMailboxImportance =
    SyncSessionMailboxImportance.MEDIUM;
  lastFetchedMailSeq = 0;

  private _specialUse: ImapMailboxSpecialUse | null = null;
  private throughputHistory: Map<TimeStamp, Throughput> = new Map<
    TimeStamp,
    Throughput
  >();
  private averageThroughputInTimeIntervalHistory: Map<
    TimeIntervalTimeStamp,
    AverageThroughput
  > = new Map<TimeIntervalTimeStamp, AverageThroughput>();
  private downloadBatchSizeHistory: Map<TimeStamp, DownloadBatchSize> = new Map<
    TimeStamp,
    DownloadBatchSize
  >();
  ...
  getAverageThroughputInTimeInterval(
    ...
    let averageThroughputInTimeInterval = getAverageOfList(
      throughputsInTimeInterval
    );
    this.averageThroughputInTimeIntervalHistory.set(
      `${fromTimeStamp}${toTimeStamp}`,
      averageThroughputInTimeInterval
    );
    return averageThroughputInTimeInterval;
  }

  getAverageEfficiencyScoreInTimeInterval(
    fromTimeStamp: TimeStamp,
    toTimeStamp: TimeStamp
  ): AverageEfficiencyScore {
    ...
    return (
      this.importance *
      (averageExists
        ? this.averageThroughputInTimeIntervalHistory.get(key)!
        : this.getAverageThroughputInTimeInterval(fromTimeStamp, toTimeStamp))
    );
  }

  getDownloadBatchSizeInTimeInterval(
    fromTimeStamp: TimeStamp,
    toTimeStamp: TimeStamp
  ): DownloadBatchSize {
    let downloadBatchSizeInTimeInterval = [
      ...this.downloadBatchSizeHistory.entries(),
    ]
  ...
  reportDownloadBatchSizeUsage(downloadBatchSize?: DownloadBatchSize) {
    ...
    this.downloadBatchSizeHistory.set(
      Date.now(),
      downloadBatchSize ? downloadBatchSize : -1
    );
  }
}
