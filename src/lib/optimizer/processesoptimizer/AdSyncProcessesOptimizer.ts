...
export class AdSyncProcessesOptimizer
  extends AdSyncOptimizer
  implements AdSyncProcessesOptimizerEventListener
{
  ...
  protected startSyncSessionProcesses(amount: number) {
    ...
    nextMailboxesToDownload.forEach((mailbox) => {
      if (!this.isExistRunningProcessForMailbox(mailbox)) {
        ...
        this.runningProcessMap.set(
          this.nextProcessId,
          new OptimizerProcess(mailbox.mailboxState.path)
        );
        this.syncSessionEventListener.onStartSyncSessionProcess(
          this.nextProcessId,
          mailbox
        );
    ...
    nextProcessIdsToDrop.forEach((processId) => {
      ...
      if (mailboxToDrop) {
        let timeToLiveIntervalMS =
          1000 *
          (mailboxToDrop.syncSessionMailbox?.timeToLive
            ? mailboxToDrop.syncSessionMailbox?.timeToLive
            : 0); // conversion to milliseconds

        // a process may run at least its timeToLiveInterval in seconds
        if (
          mailboxToDrop.processStartTime + timeToLiveIntervalMS <=
          Date.now()
        ) {
          if (mailboxToDrop.syncSessionMailbox) {
            let index = this.optimizedSyncSessionMailboxes.findIndex(
              (mailbox) => {
                return mailbox.mailboxState.path == mailboxToDrop!.mailboxPath;
              }
            );
            this.optimizedSyncSessionMailboxes[index] =
              mailboxToDrop.syncSessionMailbox;
          }
  ...
  protected nextProcessIdsToDrop(amount: number): number[] {
    ...
    return [...this.runningProcessMap.entries()]
      ...
      .sort(([_processIdA, valueA], [_processIdB, valueB]) => {
        let averageEfficiencyScoreA =
          valueA.syncSessionMailbox!.getAverageEfficiencyScoreInTimeInterval(
            currentInterval.fromTimeStamp,
            currentInterval.toTimeStamp
          );
        let averageEfficiencyScoreB =
          valueB.syncSessionMailbox!.getAverageEfficiencyScoreInTimeInterval(
            currentInterval.fromTimeStamp,
            currentInterval.toTimeStamp
          );
  ...
  protected isExistRunningProcessForMailbox(mailbox: ImapSyncSessionMailbox) {
    return Array.from(this.runningProcessMap.values()).find(
      (optimizerProcess) => {
  ...
  onMailboxFinish(
    ...
    let mailboxIndex = this.optimizedSyncSessionMailboxes.findIndex(
      (mailbox) => {
        return (
          mailbox.mailboxState.path == syncSessionMailbox.mailboxState.path
        );
      }
    );
    if (mailboxIndex != -1) {
