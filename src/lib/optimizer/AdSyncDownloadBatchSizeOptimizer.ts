...
export class AdSyncDownloadBatchSizeOptimizer extends AdSyncOptimizer {
  ...
  protected optimize(): void {
    ...
    let averageThroughputCurrent =
      this.optimizedSyncSessionMailbox.getAverageThroughputInTimeInterval(
        currentInterval.fromTimeStamp,
        currentInterval.toTimeStamp
      );
    let averageThroughputLast =
      this.optimizedSyncSessionMailbox.getAverageThroughputInTimeInterval(
        lastInterval.fromTimeStamp,
        lastInterval.toTimeStamp
      );

    let downloadBatchSizeCurrent =
      this.optimizedSyncSessionMailbox.getDownloadBatchSizeInTimeInterval(
        currentInterval.fromTimeStamp,
        currentInterval.toTimeStamp
      );
    let downloadBatchSizeLast =
      this.optimizedSyncSessionMailbox.getDownloadBatchSizeInTimeInterval(
        lastInterval.fromTimeStamp,
        lastInterval.toTimeStamp
      );
    let downloadBatchSizeDidIncrease =
      downloadBatchSizeCurrent - downloadBatchSizeLast >= 0;

    if (
      averageThroughputCurrent + THROUGHPUT_THRESHOLD >=
      averageThroughputLast
    ) {
      if (downloadBatchSizeDidIncrease) {
        this.optimizedSyncSessionMailbox.downloadBatchSize =
          this.optimizedSyncSessionMailbox.downloadBatchSize +
          this.optimizationDifference;
      } else if (
        this.optimizedSyncSessionMailbox.downloadBatchSize -
          this.optimizationDifference >
        0
