...
export abstract class AdSyncOptimizer {
  ...
  protected getCurrentTimeStampInterval(): TimeStampInterval {
    let fromTimeStamp =
      this.optimizerUpdateTimeStampHistory.at(-1) !== undefined
        ? this.optimizerUpdateTimeStampHistory.at(-1)!
        : 0;
  ...
  protected getLastTimeStampInterval(): TimeStampInterval {
    let fromTimeStamp =
      this.optimizerUpdateTimeStampHistory.at(-2) !== undefined
        ? this.optimizerUpdateTimeStampHistory.at(-2)!
        : 0;
    let toTimeStamp =
      this.optimizerUpdateTimeStampHistory.at(-1) !== undefined
        ? this.optimizerUpdateTimeStampHistory.at(-1)!
