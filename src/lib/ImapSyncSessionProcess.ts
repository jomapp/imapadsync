...
export class ImapSyncSessionProcess {
  ...
  constructor(
    ...
    this.adSyncProcessesOptimizerEventListener =
      adSyncProcessesOptimizerEventListener;
  ...
  async startSyncSessionProcess(
    ...
    try {
      ...
      if (
        ...
        error.responseStatus.match('(NO|BAD)')
      ) {
  ...
  private async runSyncSessionProcess(
    imapClient: ImapFlow,
    adSyncEventListener: AdSyncEventListener
  ) {
    ...
    try {
      ...
      let mailboxObject = await imapClient.mailboxOpen(
        this.adSyncOptimizer.optimizedSyncSessionMailbox.mailboxState.path,
        { readOnly: true }
      );

      // emit ImapMailboxStatus and update SyncSessionMailbox
      let imapMailboxStatus =
        ImapMailboxStatus.fromImapFlowMailboxObject(mailboxObject);
      adSyncEventListener.onMailboxStatus(imapMailboxStatus);
      this.updateSyncSessionMailbox(imapMailboxStatus);

      let openedImapMailbox = ImapMailbox.fromSyncSessionMailbox(
        this.adSyncOptimizer.optimizedSyncSessionMailbox
      );
      let isEnableImapQresync =
        this.adSyncConfig.isEnableImapQresync && highestModSeq != null;

      if (isEnableImapQresync) {
        this.setupImapFlowExpungeHandler(
          imapClient,
          openedImapMailbox,
          adSyncEventListener
        );
      ...
      let nextUidFetchRequest =
        await differentialUidLoader.getNextUidFetchRequest(
          this.adSyncOptimizer.optimizedSyncSessionMailbox.downloadBatchSize
        );
      ...
      while (nextUidFetchRequest) {
        ...
        this.adSyncOptimizer.optimizedSyncSessionMailbox.reportDownloadBatchSizeUsage(
          nextUidFetchRequest.usedDownloadBatchSize
        );
        ...
        for await (const mail of mails) {
          ...
          if (mail.source) {
            ...
            this.adSyncOptimizer.optimizedSyncSessionMailbox.reportCurrentThroughput(
              currenThroughput
            );
            ...
            let imapMail = await ImapMail.fromImapFlowFetchMessageObject(
              mail,
              openedImapMailbox,
              this.adSyncOptimizer.optimizedSyncSessionMailbox.mailboxState.importedUidToMailIdsMap.get(
                mail.uid
              )
            );
            ...
            switch (nextUidFetchRequest.fetchRequestType) {
              case UidFetchRequestType.CREATE:
                ...
                if (
                  this.adSyncConfig.emitAdSyncEventTypes.has(
                    AdSyncEventType.CREATE
                  )
                ) {
            ...
            adSyncEventListener.onError(
              new ImapError(
                `No IMAP mail source available for IMAP mail with UID ${mail.uid}.`
              )
            );
          }
  ...
  private async logout(
    imapClient: ImapFlow,
    isMailboxFinished: boolean,
    lastFetchedMailSeq: number = 0
  ) {
    await imapClient.logout();

    if (isMailboxFinished) {
      this.adSyncProcessesOptimizerEventListener.onMailboxFinish(
        this.processId,
        this.adSyncOptimizer.optimizedSyncSessionMailbox
      );
    } else {
      this.adSyncOptimizer.optimizedSyncSessionMailbox.lastFetchedMailSeq =
        lastFetchedMailSeq;
      this.adSyncProcessesOptimizerEventListener.onMailboxInterrupted(
        this.processId,
        this.adSyncOptimizer.optimizedSyncSessionMailbox
      );
    }
  ...
  private initFetchOptions(isEnableImapQresync: boolean) {
    ...
    if (isEnableImapQresync) {
      let highestModSeq = [
        ...this.adSyncOptimizer.optimizedSyncSessionMailbox.mailboxState.importedUidToMailIdsMap.values(),
      ].reduce<bigint>(
        (acc, imapMailIds) =>
          imapMailIds.modSeq && imapMailIds.modSeq > acc
            ? imapMailIds.modSeq
            : acc,
        BigInt(0)
      );
  ...
  private handleQresyncFetchResult(
    imapMails: ImapMail[],
    adSyncEventListener: AdSyncEventListener
  ) {
    imapMails.forEach((imapMail) => {
      let isMailUpdate =
        this.adSyncOptimizer.optimizedSyncSessionMailbox.mailboxState.importedUidToMailIdsMap.has(
          imapMail.uid
        );

      if (
        isMailUpdate &&
        this.adSyncConfig.emitAdSyncEventTypes.has(AdSyncEventType.UPDATE)
      ) {
        adSyncEventListener.onMail(imapMail, AdSyncEventType.UPDATE);
      } else if (
        this.adSyncConfig.emitAdSyncEventTypes.has(AdSyncEventType.CREATE)
      ) {
        this.adSyncOptimizer.optimizedSyncSessionMailbox.mailboxState.importedUidToMailIdsMap.set(
          imapMail.uid,
          new ImapMailIds(imapMail.uid)
  ...
  private updateSyncSessionMailbox(imapMailboxStatus: ImapMailboxStatus) {
    ...
    this.adSyncOptimizer.optimizedSyncSessionMailbox.mailCount =
      imapMailboxStatus.messageCount ?? null;
  }

  private async handleDeletedUids(
    deletedUids: number[],
    openedImapMailbox: ImapMailbox,
    adSyncEventListener: AdSyncEventListener
  ) {
    deletedUids.forEach((deletedUid) => {
      this.emitImapMailDeleteEvent(
        deletedUid,
        openedImapMailbox,
        adSyncEventListener
      );
  ...
  private setupImapFlowErrorHandler(
    imapClient: ImapFlow,
    adSyncEventListener: AdSyncEventListener
  ) {
  ...
  private setupImapFlowExpungeHandler(
    imapClient: ImapFlow,
    openedImapMailbox: ImapMailbox,
    adSyncEventListener: AdSyncEventListener
  ) {
  ...
  private emitImapMailDeleteEvent(
    ...
    if (this.adSyncConfig.emitAdSyncEventTypes.has(AdSyncEventType.DELETE)) {
      ...
      this.adSyncOptimizer.optimizedSyncSessionMailbox.mailboxState.importedUidToMailIdsMap.delete(
        deletedUid
      );
