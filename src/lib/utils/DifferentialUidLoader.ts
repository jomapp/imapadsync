...
export class DifferentialUidLoader {
  ...
  async calculateUidDiff(
    ...
    let seenUids = await this.calculateUidDiffInBatches(
      lastFetchedMailSeq + 1,
      downloadBatchSize,
      totalMessageCount
    );
  ...
  private async calculateUidDiffInBatches(
    fromSeq: number,
    downloadBatchSize: number,
    totalMessageCount: number | null
  ): Promise<SeenUids> {
    ...
    let mails = this.imapClient.fetch(
      `${fromSeqString}:${toSeqString}`,
      { uid: true },
      {}
    );
    ...
    if (!isFinalBatch) {
      seenUids.push(
        ...(await this.calculateUidDiffInBatches(
          toSeq + 1,
          downloadBatchSize,
          totalMessageCount
        ))
  ...
  async getNextUidFetchRequest(
    downloadBatchSize: number
  ): Promise<UidFetchRequest | null> {
    ...
    if (
      this.emitAdSyncEventTypes.has(AdSyncEventType.CREATE) &&
      this.uidCreateQueue.length != 0
    ) {
      nextUidFetchRange = this.uidCreateQueue.splice(0, downloadBatchSize);
      fetchRequestType = UidFetchRequestType.CREATE;
    } else if (
      this.emitAdSyncEventTypes.has(AdSyncEventType.UPDATE) &&
      this.uidUpdateQueue.length != 0
    ) {
    ...
    let uidFetchSequenceStrings =
      this.buildUidFetchSequenceStrings(nextUidFetchRange);
    let nextUidFetchSequenceRequests = uidFetchSequenceStrings.map(
      (uidFetchSequenceString) => {
        return {
  ...
  private buildUidFetchSequenceStrings(uidsToFetch: number[]): string[] {
    let uidFetchSequenceList = uidsToFetch.reduce<UidFetchSequence[]>(
      (uidFetchSequenceList, uid: number) => {
    ...
    let perChunk = 25;
    let uidFetchSequenceChunks = uidFetchSequenceList.reduce<
      UidFetchSequence[][]
    >(
      (
        uidFetchSequenceListChunks: UidFetchSequence[][],
        uidFetchSequenceList,
        index
      ) => {
    ...
    let uidFetchSequenceStrings = uidFetchSequenceChunks.map(
      (uidFetchSequenceChunk) => {
        return uidFetchSequenceChunk.reduce<string>(
          (uidFetchSequenceString, uidFetchSequence, index) => {
            if (uidFetchSequence.fromUid == uidFetchSequence.toUid) {
              return (
                uidFetchSequenceString +
                `${uidFetchSequence.fromUid}` +
