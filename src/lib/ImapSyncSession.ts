import {
  ...
  SyncSessionMailboxImportance,
} from './ImapSyncSessionMailbox.js';
...
} from './ImapSyncSessionProcess.js';
...
export class ImapSyncSession implements SyncSessionEventListener {
  ...
  private runningSyncSessionProcesses: Map<number, ImapSyncSessionProcess> =
    new Map();
  ...
  private async shutDownSyncSession(
    isPostpone: boolean,
    postponeDuration: number = DEFAULT_POSTPONE_TIME
  ) {
    this.state = SyncSessionState.PAUSED;
    ...
    if (isPostpone) {
      ...
      this.adSyncEventListener.onPostpone(
        new Date(Date.now() + postponeDuration)
      );
    }
  }
  ...
  private async runSyncSession() {
    ...
    if (mailboxes != null) {
      if (this.adSyncConfig.isEnableParallelProcessesOptimizer) {
        ...
        this.adSyncOptimizer = new AdSyncSingleProcessesOptimizer(
          mailboxes,
          this
        );
      }
  ...
  private async setupSyncSession(): Promise<ImapSyncSessionMailbox[] | null> {
    ...
    let knownMailboxes = this.imapSyncState.imapMailboxStates.map(
      (mailboxState) => {
        return new ImapSyncSessionMailbox(
          mailboxState,
    ...
    try {
      ...
      let fetchedRootMailboxes = listTreeResponse.folders.map(
        (listTreeResponse) => {
          return ImapMailbox.fromImapFlowListTreeResponse(
            listTreeResponse,
            null
          );
        }
  ...
  private getSyncSessionMailboxes(
    knownMailboxes: ImapSyncSessionMailbox[],
    fetchedRootMailboxes: ImapMailbox[]
  ): ImapSyncSessionMailbox[] {
    let resultMailboxes: ImapSyncSessionMailbox[] = [];
    fetchedRootMailboxes.forEach((fetchedRootMailbox) => {
      resultMailboxes.push(
        ...this.traverseImapMailboxes(knownMailboxes, fetchedRootMailbox)
      );
    });
    ...
    knownMailboxes.map((knownMailbox) => {
      ...
      if (index == -1) {
        let deletedImapMailbox =
          ImapMailbox.fromSyncSessionMailbox(knownMailbox);
        this.adSyncEventListener.onMailbox(
          deletedImapMailbox,
          AdSyncEventType.DELETE
        );
        return true;
      }
  ...
  private traverseImapMailboxes(
    knownMailboxes: ImapSyncSessionMailbox[],
    imapMailbox: ImapMailbox
  ): ImapSyncSessionMailbox[] {
    let result: ImapSyncSessionMailbox[] = [];

    let syncSessionMailbox = knownMailboxes.find(
      (value) => value.mailboxState.path == imapMailbox.path
    );
  ...
  onStartSyncSessionProcess(
    processId: number,
    nextMailboxToDownload: ImapSyncSessionMailbox
  ): void {
    if (this.state == SyncSessionState.RUNNING) {
      console.log(
        'onStartSyncSessionProcess : processId: ' +
          processId +
          ' -> ' +
          nextMailboxToDownload.mailboxState.path
      );

      if (!this.adSyncOptimizer) {
        throw new Error(
          'The SyncSessionEventListener should be exclusively used by the AdSyncEfficiencyScoreOptimizer!'
        );
      }
      ...
      let adSyncDownloadBlatchSizeOptimizer =
        new AdSyncDownloadBatchSizeOptimizer(
      ...
      this.runningSyncSessionProcesses.set(
        syncSessionProcess.processId,
        syncSessionProcess
      );
      syncSessionProcess
        .startSyncSessionProcess(
          this.imapSyncState.imapAccount,
          this.adSyncEventListener
        )
        .then((state) => {
          if (state == SyncSessionProcessState.CONNECTION_FAILED_REJECTED) {
            ...
            state == SyncSessionProcessState.CONNECTION_FAILED_UNKNOWN
          ) {
            this.adSyncOptimizer?.forceStopSyncSessionProcess(processId, false);
          } else {
            if (
              this.adSyncConfig.isEnableDownloadBatchSizeOptimizer &&
              this.state == SyncSessionState.RUNNING
            ) {
  ...
  onStopSyncSessionProcess(nextProcessIdToDrop: number): void {
    ...
    let syncSessionProcessToDrop =
      this.runningSyncSessionProcesses.get(nextProcessIdToDrop);
  ...
  onDownloadQuotaUpdate(downloadedQuota: number): void {
    ...
    let downloadedQuotaTotal = this.downloadedQuotas.reduce(
      (quotaSum, quota) => quotaSum + quota,
      0
    );
    if (
      downloadedQuotaTotal >
      this.imapSyncState.maxQuota - DOWNLOADED_QUOTA_SAFETY_THRESHOLD
    ) {
